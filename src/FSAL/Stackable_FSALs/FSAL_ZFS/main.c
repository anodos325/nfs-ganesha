...
static fsal_status_t init_config(struct fsal_module *zfs_fsal_module,
				 ...
				 struct config_error_type *err_type)
{
	...
	display_fsinfo(zfs_fsal_module);
	LogDebug(COMPONENT_FSAL,
		 "FSAL INIT: Supported attributes mask = 0x%" PRIx64,
		 zfs_fsal_module->fs_info.supported_attrs);
...
{
	int retval;
	struct fsal_module *myself = &ZFSFS.module;
	...
	myself->m_ops.update_export = zfs_update_export;
	myself->m_ops.init_config = init_config;

	/* Initialize the fsal_obj_handle ops for FSAL NULL */
	zfs_handle_ops_init(&ZFSFS.handle_ops);
}
...
MODULE_FINI void zfs_unload(void)
{
	...
	retval = unregister_fsal(&ZFSFS.module);
	if (retval != 0) {
