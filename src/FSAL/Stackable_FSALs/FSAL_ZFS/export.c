...
static struct state_t *zfs_alloc_state(struct fsal_export *exp_hdl,
...
static void zfs_free_state(struct fsal_export *exp_hdl,
			      struct state_t *state)
{
	struct zfs_fsal_export *exp = container_of(exp_hdl,
...
static bool zfs_is_superuser(struct fsal_export *exp_hdl,
...
static fsal_status_t zfs_host_to_key(struct fsal_export *exp_hdl,
					  struct gsh_buffdesc *fh_desc)
{
	struct zfs_fsal_export *exp =
		container_of(exp_hdl, struct zfs_fsal_export, export);
...
static void zfs_prepare_unexport(struct fsal_export *exp_hdl)
{
	struct zfs_fsal_export *exp =
		container_of(exp_hdl, struct zfs_fsal_export, export);
...
{
	ops->release = release;
	ops->prepare_unexport = zfs_prepare_unexport;
	ops->lookup_path = zfs_lookup_path;
	ops->wire_to_host = wire_to_host;
	ops->host_to_key = zfs_host_to_key;
	...
	ops->alloc_state = zfs_alloc_state;
	ops->free_state = zfs_free_state;
	ops->is_superuser = zfs_is_superuser;
}

struct zfs_fsal_args {
				   ...
				   const struct fsal_up_vector *up_ops)
{
	...
	fsal_stack = lookup_fsal(zfs_fsal.subfsal.name);
	if (fsal_stack == NULL) {
		LogMajor(COMPONENT_FSAL,
			 "zfs create export failed to lookup for FSAL %s",
			 zfs_fsal.subfsal.name);
		return fsalstat(ERR_FSAL_INVAL, EINVAL);
	}

	myself = gsh_calloc(1, sizeof(struct zfs_fsal_export));
	expres = fsal_stack->m_ops.create_export(fsal_stack,
						 zfs_fsal.subfsal.fsal_node,
						 err_type,
						 up_ops);
	fsal_put(fsal_stack);
...
fsal_status_t zfs_update_export(struct fsal_module *fsal_hdl,
				   void *parse_node,
				   struct config_error_type *err_type,
				   ...
				   struct fsal_module *updated_super)
{
	...
	if (fsal_stack == NULL) {
		LogMajor(COMPONENT_FSAL,
			 "zfs update export failed to lookup for FSAL %s",
			 zfs_fsal.subfsal.name);
		return fsalstat(ERR_FSAL_INVAL, EINVAL);
	}
