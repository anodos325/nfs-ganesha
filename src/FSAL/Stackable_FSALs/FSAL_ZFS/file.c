...
/**
 * @brief Callback arg for ZFS async callbacks
 *
 * ZFS needs to know what its object is related to the sub-FSAL's object.
 * This wraps the given callback arg with ZFS specific info
 */
struct zfs_async_arg {
...
/**
 * @brief Callback for ZFS async calls
 *
 * Unstack, and call up.
 *
 ...
 */
void zfs_async_cb(struct fsal_obj_handle *obj, fsal_status_t ret,
		   void *obj_data, void *caller_data)
{
	struct fsal_export *save_exp = op_ctx->fsal_export;
	struct zfs_async_arg *arg = caller_data;
...
fsal_status_t zfs_open2(struct fsal_obj_handle *obj_hdl,
			   ...
			   bool *caller_perm_check)
{
	...
	if (sub_handle) {
		/* wrap the subfsal handle in a zfs handle. */
		return zfs_alloc_and_check_handle(export, sub_handle,
						     obj_hdl->fs, new_obj,
						     status);
	}
...
bool zfs_check_verifier(struct fsal_obj_handle *obj_hdl,
			   fsal_verifier_t verifier)
{
	struct zfs_fsal_obj_handle *handle =
		container_of(obj_hdl, struct zfs_fsal_obj_handle,
...
fsal_openflags_t zfs_status2(struct fsal_obj_handle *obj_hdl,
				struct state_t *state)
{
	struct zfs_fsal_obj_handle *handle =
		container_of(obj_hdl, struct zfs_fsal_obj_handle,
...
fsal_status_t zfs_reopen2(struct fsal_obj_handle *obj_hdl,
			     struct state_t *state,
			     fsal_openflags_t openflags)
{
	struct zfs_fsal_obj_handle *handle =
...
void zfs_read2(struct fsal_obj_handle *obj_hdl,
		  bool bypass,
		  ...
		  void *caller_arg)
{
	...
	handle->sub_handle->obj_ops->read2(handle->sub_handle, bypass,
					  zfs_async_cb, read_arg, arg);
	op_ctx->fsal_export = &export->export;
}

void zfs_write2(struct fsal_obj_handle *obj_hdl,
		   bool bypass,
		   fsal_async_cb done_cb,
		   ...
		   void *caller_arg)
{
	...
	handle->sub_handle->obj_ops->write2(handle->sub_handle, bypass,
					   zfs_async_cb, write_arg, arg);
	op_ctx->fsal_export = &export->export;
}

fsal_status_t zfs_seek2(struct fsal_obj_handle *obj_hdl,
			   struct state_t *state,
			   struct io_info *info)
{
	struct zfs_fsal_obj_handle *handle =
...
fsal_status_t zfs_io_advise2(struct fsal_obj_handle *obj_hdl,
				struct state_t *state,
...
fsal_status_t zfs_commit2(struct fsal_obj_handle *obj_hdl, off_t offset,
			     size_t len)
{
	struct zfs_fsal_obj_handle *handle =
		container_of(obj_hdl, struct zfs_fsal_obj_handle,
...
fsal_status_t zfs_lock_op2(struct fsal_obj_handle *obj_hdl,
			      struct state_t *state,
			      void *p_owner,
...
fsal_status_t zfs_close2(struct fsal_obj_handle *obj_hdl,
			    struct state_t *state)
{
	struct zfs_fsal_obj_handle *handle =
		container_of(obj_hdl, struct zfs_fsal_obj_handle,
...
fsal_status_t zfs_fallocate(struct fsal_obj_handle *obj_hdl,
