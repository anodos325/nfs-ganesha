 ...
 */
static struct zfs_fsal_obj_handle *zfs_alloc_handle(
		struct zfs_fsal_export *export,
		struct fsal_obj_handle *sub_handle,
		struct fsal_filesystem *fs)
{
	struct zfs_fsal_obj_handle *result;

	result = gsh_calloc(1, sizeof(struct zfs_fsal_obj_handle));

	/* default handlers */
	fsal_obj_handle_init(&result->obj_handle, &export->export,
			     sub_handle->type);
	/* zfs handlers */
	result->obj_handle.obj_ops = &ZFSFS.handle_ops;
...
/**
 * Attempts to create a new zfs handle, or cleanup memory if it fails.
 *
 * This function is a wrapper of zfs_alloc_handle. It adds error checking
 ...
 */
fsal_status_t zfs_alloc_and_check_handle(
		struct zfs_fsal_export *export,
		struct fsal_obj_handle *sub_handle,
		struct fsal_filesystem *fs,
		...
		fsal_status_t subfsal_status)
{
	...
	if (!FSAL_IS_ERROR(subfsal_status)) {
		struct zfs_fsal_obj_handle *null_handle;

		null_handle = zfs_alloc_handle(export, sub_handle, fs);
...
{
	/** Parent as zfs handle.*/
	struct zfs_fsal_obj_handle *null_parent =
	...
	*handle = NULL;
	...
	/** Current zfs export. */
	struct zfs_fsal_export *export =
		container_of(op_ctx->fsal_export, struct zfs_fsal_export,
			     export);
	...
	/* wraping the subfsal handle in a zfs handle. */
	return zfs_alloc_and_check_handle(export, sub_handle, parent->fs,
					     handle, status);
}
...
{
	*new_obj = NULL;
	/** Parent directory zfs handle. */
	struct zfs_fsal_obj_handle *parent_hdl =
		container_of(dir_hdl, struct zfs_fsal_obj_handle,
			     obj_handle);
	/** Current zfs export. */
	struct zfs_fsal_export *export =
		container_of(op_ctx->fsal_export, struct zfs_fsal_export,
	...
	/* wraping the subfsal handle in a zfs handle. */
	return zfs_alloc_and_check_handle(export, sub_handle, dir_hdl->fs,
					     new_obj, status);
}
...
{
	/** Parent directory zfs handle. */
	struct zfs_fsal_obj_handle *zfs_dir =
		container_of(dir_hdl, struct zfs_fsal_obj_handle,
			     obj_handle);
	/** Current zfs export. */
	struct zfs_fsal_export *export =
	...
	*new_obj = NULL;
	...
	op_ctx->fsal_export = export->export.sub_export;
	fsal_status_t status = zfs_dir->sub_handle->obj_ops->mknode(
		zfs_dir->sub_handle, name, nodetype, attrs_in,
				 ...
				 struct attrlist *attrs_out)
{
	...
	*new_obj = NULL;
	...
	op_ctx->fsal_export = export->export.sub_export;
	fsal_status_t status = zfs_dir->sub_handle->obj_ops->symlink(
		zfs_dir->sub_handle, name, link_path, attrs_in, &sub_handle,
		attrs_out);
	op_ctx->fsal_export = &export->export;
...
static fsal_status_t readsymlink(struct fsal_obj_handle *obj_hdl,
				 ...
				 bool refresh)
{
	struct zfs_fsal_obj_handle *handle =
		(struct zfs_fsal_obj_handle *) obj_hdl;
	struct zfs_fsal_export *export =
		container_of(op_ctx->fsal_export, struct zfs_fsal_export,
			     export);
			      ...
			      const char *name)
{
	struct zfs_fsal_obj_handle *handle =
		(struct zfs_fsal_obj_handle *) obj_hdl;
	struct zfs_fsal_obj_handle *zfs_dir =
		(struct zfs_fsal_obj_handle *) destdir_hdl;
	struct zfs_fsal_export *export =
		container_of(op_ctx->fsal_export, struct zfs_fsal_export,
			     export);
	...
	fsal_status_t status = handle->sub_handle->obj_ops->link(
		handle->sub_handle, zfs_dir->sub_handle, name);
	op_ctx->fsal_export = &export->export;
 ...
 * @param dir_state A zfs_readdir_state struct.
 * @param cookie Directly passed to upper layer.
 *
 * @return Result coming from the upper layer.
 */
static enum fsal_dir_result zfs_readdir_cb(
					const char *name,
					struct fsal_obj_handle *sub_handle,
					struct attrlist *attrs,
					void *dir_state, fsal_cookie_t cookie)
{
	struct zfs_readdir_state *state =
		(struct zfs_readdir_state *) dir_state;
	struct fsal_obj_handle *new_obj;

	if (FSAL_IS_ERROR(zfs_alloc_and_check_handle(state->exp, sub_handle,
				  ...
				  bool *eof)
{
	...
	struct zfs_readdir_state cb_state = {
		.cb = cb,
		.dir_state = dir_state,
		.exp = export
	};
	...
	fsal_status_t status =
		...
		whence, &cb_state, zfs_readdir_cb, attrmask, eof);
	op_ctx->fsal_export = &export->export;
				     ...
				     const char *name)
{
	...
	struct zfs_fsal_obj_handle *handle =
		container_of(parent, struct zfs_fsal_obj_handle,
			     obj_handle);
...
static fsal_status_t renamefile(struct fsal_obj_handle *obj_hdl,
				...
				const char *new_name)
{
	struct zfs_fsal_obj_handle *zfs_olddir =
		container_of(olddir_hdl, struct zfs_fsal_obj_handle,
			     obj_handle);
	struct zfs_fsal_obj_handle *zfs_newdir =
		container_of(newdir_hdl, struct zfs_fsal_obj_handle,
			     obj_handle);
	struct zfs_fsal_obj_handle *zfs_obj =
		container_of(obj_hdl, struct zfs_fsal_obj_handle,
			     obj_handle);
	...
	/* calling subfsal method */
	op_ctx->fsal_export = export->export.sub_export;
	fsal_status_t status = zfs_olddir->sub_handle->obj_ops->rename(
		zfs_obj->sub_handle, zfs_olddir->sub_handle,
		old_name, zfs_newdir->sub_handle, new_name);
	op_ctx->fsal_export = &export->export;
...
static fsal_status_t zfs_setattr2(struct fsal_obj_handle *obj_hdl,
				     bool bypass,
				 ...
				 const char *name)
{
	...
	/* calling subfsal method */
	op_ctx->fsal_export = export->export.sub_export;
	fsal_status_t status = zfs_dir->sub_handle->obj_ops->unlink(
		zfs_dir->sub_handle, zfs_obj->sub_handle, name);
	op_ctx->fsal_export = &export->export;
...
static void release(struct fsal_obj_handle *obj_hdl)
{
	...
	/* cleaning data allocated by zfs */
	fsal_obj_handle_fini(&hdl->obj_handle);
	gsh_free(hdl);
}

static bool zfs_is_referral(struct fsal_obj_handle *obj_hdl,
			       struct attrlist *attrs,
			       bool cache_attrs)
{
	struct zfs_fsal_obj_handle *hdl =
...
void zfs_handle_ops_init(struct fsal_obj_ops *ops)
{
	...
	/* Multi-FD */
	ops->open2 = zfs_open2;
	ops->check_verifier = zfs_check_verifier;
	ops->status2 = zfs_status2;
	ops->reopen2 = zfs_reopen2;
	ops->read2 = zfs_read2;
	ops->write2 = zfs_write2;
	ops->seek2 = zfs_seek2;
	ops->io_advise2 = zfs_io_advise2;
	ops->commit2 = zfs_commit2;
	ops->lock_op2 = zfs_lock_op2;
	ops->setattr2 = zfs_setattr2;
	ops->close2 = zfs_close2;
	ops->fallocate = zfs_fallocate;

	/* xattr related functions */
	ops->list_ext_attrs = zfs_list_ext_attrs;
	ops->getextattr_id_by_name = zfs_getextattr_id_by_name;
	ops->getextattr_value_by_name = zfs_getextattr_value_by_name;
	ops->getextattr_value_by_id = zfs_getextattr_value_by_id;
	ops->setextattr_value = zfs_setextattr_value;
	ops->setextattr_value_by_id = zfs_setextattr_value_by_id;
	ops->remove_extattr_by_id = zfs_remove_extattr_by_id;
	ops->remove_extattr_by_name = zfs_remove_extattr_by_name;

	ops->is_referral = zfs_is_referral;
}
				 ...
				 struct attrlist *attrs_out)
{
	...
	*handle = NULL;
	...
	struct zfs_fsal_export *exp =
		container_of(exp_hdl, struct zfs_fsal_export, export);
	...
	/* wraping the subfsal handle in a zfs handle. */
	/* Note : zfs filesystem = subfsal filesystem or NULL ? */
	return zfs_alloc_and_check_handle(exp, sub_handle, NULL, handle,
					     status);
}
				   ...
				   struct attrlist *attrs_out)
{
	...
	*handle = NULL;
	...
	/* wraping the subfsal handle in a zfs handle. */
	/* Note : zfs filesystem = subfsal filesystem or NULL ? */
	return zfs_alloc_and_check_handle(export, sub_handle, NULL, handle,
					     status);
}
